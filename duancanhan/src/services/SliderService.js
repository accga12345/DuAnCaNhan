...
export const createSlider = async (data, access_token) => {
    const res = await axiosJwt.post(`${import.meta.env.VITE_API_URL}/slider/add`, data, {
        headers: {
...
export const deleteSlider = async (id, access_token) => {
    const res = await axiosJwt.delete(`${import.meta.env.VITE_API_URL}/slider/delete/${id}`, {
        headers: {
            Authorization: `Bearer ${access_token}`,
        }
    })
