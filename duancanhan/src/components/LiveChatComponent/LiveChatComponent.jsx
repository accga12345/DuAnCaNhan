...
const LiveChatComponent = () => {
    ...
    useEffect(() => {
        ...
        newSocket.on('receive_chat_message', (data) => {
            setMessages(prev => {
                ...
                // Cần so sánh với cả user._id và socket.id hiện tại để nhận diện chính mình (đặc biệt là khách vãng lai)
    ...
    if (user?.isAdmin || user?.isEmployee) return null;
    ...
    return (
        <div style={{ position: 'fixed', bottom: 90, right: 20, zIndex: 9999, fontFamily: 'Arial' }}>
            ...
            {isOpen && (
                <div style={{ position: 'absolute', bottom: 70, right: 0, width: 350, height: 500, background: '#fff', borderRadius: 15, boxShadow: '0 10px 30px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    <div style={{ padding: 15, background: '#52c41a', color: '#fff', textAlign: 'center', fontWeight: 'bold', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span>Chat với Nhân Viên</span>
                        <button onClick={handleEndChat} style={{ background: 'transparent', border: 'none', color: 'white', cursor: 'pointer', fontSize: 16 }}>✕</button>
                    </div>
